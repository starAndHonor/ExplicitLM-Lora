...
class ReferenceCompatModel(nn.Module):
    """Compatibility wrapper exposing the old Reference eval interface.

    The copied Reference evaluation code expects:
    - create_model(...)
    - model.load_injection_weights(path)
    - model(input_ids, knowledge_ids) -> logits tensor

    This wrapper adapts the current repo's ModifiedQwen implementation to that
    contract while preserving the copied Reference evaluation logic.
    """
    ...
    def __init__(
        ...
        if knowledge_adapter:
            log_msg("WARNING", "knowledge_adapter=True is ignored in e2-ref compat loader")
    ...
    def load_injection_weights(self, load_dir: str) -> None:
        ...
        if (load_path / "encoder_layers.pt").exists() or (load_path / "encoder_norm.pt").exists():
            log_msg(
                "INFO",
                f"e2-ref compat loader only restores injection_modules.pt from {load_path}",
            )
